...
export const usePagination= ({setContent,getContentCB,filtersParams,refetchOnMountOrArgChange = false}:usePagintaionParams) =>
{
    const dispatch = useAppDispatch();

    
    const [filters, setFilters] = useState({...initialFilters,...filtersParams});
    const [pagesCount,setPagesCount] = useState(0);

    const getContentResult = getContentCB({filters},{refetchOnMountOrArgChange:refetchOnMountOrArgChange});
