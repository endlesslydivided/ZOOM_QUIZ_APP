...
describe('PlaySessionsController', () => {
  ...
  describe('getResults', () => {
    ...
    it('should get play sessions results', async () => {
      const playSessionResults = await controller.getPlaySessionsResults(
        zoomContext,
        query,
      );

      expect(
        mockPlaySessionsService.getPlaySessionsResults,
      ).toHaveBeenCalledTimes(1);
      expect(
        mockPlaySessionsService.getPlaySessionsResults,
      ).toHaveBeenCalledWith(query, zoomContext);
  ...
  describe('getPlaySessionReport', () => {
    ...
    it('should get one play session report', async () => {
      const playSessionReport = await controller.getPlaySessionReport(
        playSessionId,
      );

      expect(
        mockPlaySessionsService.getPlaySessionReport,
      ).toHaveBeenCalledTimes(1);
