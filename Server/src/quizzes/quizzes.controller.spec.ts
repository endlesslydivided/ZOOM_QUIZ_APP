...
describe('QuizzesController', () => {
  ...
  describe('createQuiz', () => {
    ...
    it('should create a quiz', async () => {
      const quizResult: Quiz = await controller.createQuiz(
        quizDto,
        zoomContext,
      );

      expect(mockQuizzesService.createQuiz).toHaveBeenCalledTimes(1);
      expect(mockQuizzesService.createQuiz).toBeCalledWith(
        quizDto,
        zoomContext,
      );
  ...
  describe('getUserQuizzes', () => {
    ...
    it('should get user quizzes', async () => {
      const userQuizzesResult = await controller.getUserQuizzes(
        zoomContext,
        filters,
      );

      expect(mockQuizzesService.getUserQuizzes).toHaveBeenCalledTimes(1);
      expect(mockQuizzesService.getUserQuizzes).toBeCalledWith(
        filters,
        zoomContext,
      );
